...
function ProjectCard({ project, editing, onUpdate, onRemove, onUploadImage }: ProjectCardProps) {
  ...
  function updateTag(index: number, value: string) {
    ...
    if (!value.trim()) next.splice(index, 1);
    else next[index] = value.trim();
    onUpdate({ tags: next });
  }

  return (
    <article className="widget-gallery-cards-card">
      ...
      <EditableText
        as="h3"
        className="widget-gallery-cards-title"
        value={project.title}
      ...
      {(editing || project.date) && (
        <EditableText
          as="span"
          className="widget-gallery-cards-date"
          value={project.date}
          placeholder="Date"
      ...
      {(editing || project.description) && (
        <EditableText
          as="p"
          className="widget-gallery-cards-desc"
      ...
      {(editing || project.tags.length > 0) && (
        <div className="widget-gallery-cards-tags">
          {project.tags.map((tag, i) => (
            <span key={i} className="widget-gallery-cards-tag">
      ...
      {editing ? (
        ...
        project.links[0] && (
          <a href={project.links[0]} target="_blank" rel="noreferrer" className="widget-gallery-cards-link">
            View project →
          </a>
        )
      )}
...
export function GalleryCardsComponent({ data, config }: WidgetProps) {
  ...
  return (
    <div className="widget widget-gallery-cards">
      <h2 className="widget-heading">{heading}</h2>
      {projects.length > 0 || editing ? (
        <div className="widget-gallery-cards-grid">
